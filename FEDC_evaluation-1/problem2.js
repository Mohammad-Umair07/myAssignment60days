...
function groupAndSortProducts(product){
    ...
    obj.Electronics = product.filter((ele,ind)=>{
       return ele.category == "Electronics";
    })
   
    obj.Fashion = product.filter((ele)=>ele.category=="Fashion");
    obj.Furniture = product.filter((ele)=>ele.category=="Furniture");
    obj.Photography = product.filter((ele)=>ele.category=="Photography");
    return obj;
...
let products = [
   ...
   let groupedAndSortedProducts = groupAndSortProducts(products);
